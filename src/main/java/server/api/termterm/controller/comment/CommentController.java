...
public class CommentController {
    ...
    @ApiResponses({
            @io.swagger.annotations.ApiResponse(code = 2061, message = "나만의 용어 설명 등록 완료 (200)"),
            @io.swagger.annotations.ApiResponse(code = 4052, message = "단어가 존재하지 않습니다. (400)"),
    })
    @PostMapping("/comment")
    public ApiResponse<String> registerComment(
        ...
        return ApiResponse.of(CommentResponseType.POST_SUCCESS);
    }

    @ApiOperation(value = "좋아요", notes = "나만의 용어 설명 좋아요")
    @ApiResponses({
            @io.swagger.annotations.ApiResponse(code = 2062, message = "용어 설명 좋아요 성공 (200)"),
            @io.swagger.annotations.ApiResponse(code = 4061, message = "해당하는 나만의 용어 설명이 존재하지 않습니다. (400)"),
            @io.swagger.annotations.ApiResponse(code = 4062, message = "사용자가 용어 설명에 이미 좋아요를 눌렀습니다. (400)"),
    })
    @PutMapping("/comment/like/{id}")
    public ApiResponse<String> likeComment(
            @Parameter(name = "Authorization", description = "Bearer {access-token}", in = HEADER, required = true) @RequestHeader(name = "Authorization") String token,
            @Parameter(name = "id", description = "comment id", in = PATH)  @PathVariable Long id
    ) {
        Member member = memberService.getMemberByToken(token);
        commentService.like(member, id);

        return ApiResponse.of(CommentResponseType.LIKE_SUCCESS);
    }

    @ApiOperation(value = "좋아요 취소", notes = "나만의 용어 설명 좋아요 취소")
    @ApiResponses({
            @io.swagger.annotations.ApiResponse(code = 2063, message = "용어 설명 좋아요 취소 성공 (200)"),
            @io.swagger.annotations.ApiResponse(code = 4061, message = "해당하는 나만의 용어 설명이 존재하지 않습니다. (400)"),
            @io.swagger.annotations.ApiResponse(code = 4063, message = "사용자가 용어 설명에 좋아요를 누르지 않았습니다. (400)"),
    })
    @PutMapping("/comment/dislike/{id}")
    public ApiResponse<String> dislikeComment(
            @Parameter(name = "Authorization", description = "Bearer {access-token}", in = HEADER, required = true) @RequestHeader(name = "Authorization") String token,
            @Parameter(name = "id", description = "comment id", in = PATH)  @PathVariable Long id
    ) {
        Member member = memberService.getMemberByToken(token);
        ...
        return ApiResponse.of(CommentResponseType.DISLIKE_SUCCESS);
    }

    @ApiOperation(value = "나만의 용어 설명 승인", notes = "나만의 용어 설명 승인\n관리자 권한일 때만")
    @ApiResponses({
            @io.swagger.annotations.ApiResponse(code = 2064, message = "나만의 용어 설명 승인 완료 (200)"),
    })
    @PutMapping("/comment/accept/{id}")
    public ApiResponse<String> acceptComment(
            @Parameter(name = "Authorization", description = "Bearer {access-token}", in = HEADER, required = true) @RequestHeader(name = "Authorization") String token,
        ...
        return ApiResponse.of(CommentResponseType.ACCEPT_SUCCESS);
    }
    ...
    @ApiOperation(value = "신고하기", notes = "나만의 용어설명 신고하기\n" +
            ...
            "    OTHER(\"OTEHR\", \"기타\"),")
    @ApiResponses({
            @io.swagger.annotations.ApiResponse(code = 2071, message = "신고 접수 완료 (200)"),
            @io.swagger.annotations.ApiResponse(code = 4071, message = "신고 유형이 존재하지 않습니다. (400)"),
    })
    ...
    public ApiResponse<String> reportComment(
        ...
        return ApiResponse.of(ReportResponseType.REPORT_WAITING);
    }

    @ApiOperation(value = "신고 처리 완료 (관리자)", notes = "관리자가 신고 내용 처리 완료")
    @ApiResponses({
            @io.swagger.annotations.ApiResponse(code = 2073, message = "신고 처리 완료 (200")
    })
    @PutMapping("/comment/report/completed/{id}")
    public ApiResponse<String> completeComment(
            @Parameter(name = "Authorization", description = "Bearer {access-token}", in = HEADER, required = true) @RequestHeader(name = "Authorization") String token,
            @Parameter(name = "id", description = "comment id", in = PATH)  @PathVariable(name = "id") Long id
    ){
        Member member = memberService.getMemberByToken(token);
        ...
        reportService.completeReport(member, commentService.findById(id));

        return ApiResponse.of(ReportResponseType.REPORT_COMPLETED);
    }
}
