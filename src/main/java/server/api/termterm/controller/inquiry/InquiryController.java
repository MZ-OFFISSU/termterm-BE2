...
public class InquiryController {
    ...
    @ApiOperation(value = "문의사항 접수", notes = "문의사항 접수\n" +
            ...
            "    OTHER(\"OTHER\", \"기타 문의\")")
    @ApiResponses({
            @io.swagger.annotations.ApiResponse(code = 2041, message = "문의 접수 완료 (201)"),
            @io.swagger.annotations.ApiResponse(code = 4044, message = "문의 유형이 존재하지 않습니다. (400)"),
    })
    @PostMapping("/inquiry")
    public ApiResponse<InquiryRequestDto> registerInquiry(
        ...
        return ApiResponse.of(InquiryResponseType.INQUIRY_ACCEPTED, inquiryRequestDto);
    }

    @ApiOperation(value = "문의사항 답변 완료", notes = "문의사항 답변 완료")
    @ApiResponses({
            @io.swagger.annotations.ApiResponse(code = 2042, message = "문의 답변 처리 완료 (200)"),
            @io.swagger.annotations.ApiResponse(code = 4045, message = "문의가 존재하지 않거나 삭제되었습니다. (404)"),
    })
    @PutMapping("/inquiry/to-completed/{id}")
    public ApiResponse<String> completeInquiry(
        ...
        return ApiResponse.of(InquiryResponseType.INQUIRY_COMPLETED);
    }

    @ApiOperation(value = "문의사항 대기 중 변환", notes = "문의사항 대기 중 변환")
    @ApiResponses({
            @io.swagger.annotations.ApiResponse(code = 2043, message = "문의 대기 중 변환 완료 (200)"),
            @io.swagger.annotations.ApiResponse(code = 4045, message = "문의가 존재하지 않거나 삭제되었습니다. (404)"),
    })
    @PutMapping("/inquiry/to-waiting/{id}")
    public ApiResponse<String> waitInquiry(
            @Parameter(name = "Authorization", description = "Bearer {access-token}", in = HEADER, required = true) @RequestHeader(name = "Authorization") String token,
            @Parameter(name = "id", description = "Inquiry Id", in = PATH, required = true) @PathVariable Long id
    ){
        Member member = memberService.getMemberByToken(token);
        ...
        return ApiResponse.of(InquiryResponseType.INQUIRY_HOLD);
    }
}
